...
"""Tests for the build-time runner in pcons.util.pybuilder."""
...
class TestMain:
    ...
    def test_argv_defaults_to_sys_argv(
        ...
        monkeypatch.setattr(
            sys,
            "argv",
            ["pybuilder.py", module, args, "--n-targets", "1", str(target), "a.txt"],
        )
...
class TestScriptEntryPoint:
    ...
    ``-m pcons.util.pybuilder`` would execute ``pcons/__init__.py`` first,
