...
class TestFormatNodeCommand:
    def test_indexed_targets_from_the_outputs_dict(self, test_project):
        """MSVC-style multi-output edges (DLL + import lib) keep their
        indexed TargetPath tokens: /IMPLIB: must not silently vanish."""
        from pathlib import Path

        from pcons.core.explain import format_node_command
        from pcons.core.node import FileNode
        from pcons.core.subst import SourcePath, TargetPath

        node = FileNode(Path("build/wrapper.dll"))
        node._build_info = {
            "tool": "link",
            "command_var": "sharedcmd",
            "sources": [FileNode(Path("build/obj/wrapper.obj"))],
            "outputs": {
                "primary": {"path": Path("build/wrapper.dll"), "suffix": ".dll"},
                "import_lib": {"path": Path("build/wrapper.lib"), "suffix": ".lib"},
            },
            "command": [
                "link",
                "/DLL",
        ...
        command = format_node_command(node)
        assert command is not None
        assert "/OUT:build/wrapper.dll" in command
        assert "/IMPLIB:build/wrapper.lib" in command
