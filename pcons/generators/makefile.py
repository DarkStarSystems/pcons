...
class MakefileGenerator(BaseGenerator):
    ...
    def _write_build_rule(
        ...
        if isinstance(depfile, PathToken):
            self._depfiles.add(self._depfile_path(node, depfile))
    ...
    def _write_depfile_includes(self, f: TextIO) -> None:
        ...
        make rebuilds against. Missing ones (not built yet) are skipped."""
        if not self._depfiles:
            return

        f.write("# Dependencies the commands discovered\n")
        for depfile in sorted(self._depfiles):
