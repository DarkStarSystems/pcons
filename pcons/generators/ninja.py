...
class NinjaGenerator(BaseGenerator):
    ...
    def _write_header(self, f: TextIO, project: Project) -> None:
        ...
        # Scanner scopes split discovered deps across per-target dyndep
        # files; a file may then require a path another file declares as an
        # implicit output, which ninja resolves reliably from 1.11 on.
        version = "1.11" if getattr(project, "_scan_scopes", None) else "1.10"
        f.write(f"ninja_required_version = {version}\n")
        f.write("\n")
