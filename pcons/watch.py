...
#: Exact filenames to ignore. ``compile_commands.json`` is a symlink pcons
#: maintains at the project root, so reacting to it would rebuild forever.
IGNORED_NAMES = frozenset({".DS_Store", "4913", "compile_commands.json"})
...
def make_ignore(excluded_dirs: Sequence[Path] = ()) -> Callable[[Path], bool]:
...
def watch_and_build(
    ...
    *changes* supplies batches of changed paths, defaulting to ``watchfiles``;
    ...
    ignored = make_ignore(excluded_dirs)
