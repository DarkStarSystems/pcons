...
- **Qt support hardening** (from an adversarial review with hands-on reproductions, plus a Windows pass against Qt 6.4.2): moc now sees the environment's own defines/include dirs and the compiler's real predefines (feature-gated `Q_OBJECT` under `-DFEATURE` mis-scanned before); the automoc scanner follows angle-bracket includes, headers listed in `sources=` (the CMake convention — previously they leaked onto the link line), public include dirs of linked project libraries, and out-of-project directories given as include dirs (sibling repos), while never scanning imported packages; Windows/MSVC platform flags and debug `d`-suffixes now apply per-module across repeated `find_qt()` calls; `qt_root` is a hard constraint (restricts pkg-config via `PKG_CONFIG_LIBDIR`, errors when nonexistent) instead of a hint that could silently fall back to another Qt; multiple `C:\Qt` versions sort numerically; generated `.qrc` XML is escaped; `QML_ELEMENT` classes declared in `.cpp` files register too; Qt errors join the `PconsError` hierarchy and the CLI reports them without a traceback.
- **Qt QML, translations, and deployment**: `project.QtQmlModule(...)` builds a QML module in one call — QML_ELEMENT C++ types register via moc's JSON metadata and `qmltyperegistrar`, the `qmldir` is synthesized, and QML files embed under `:/qt/qml/<uri>/` so `engine.loadFromModule(uri, "Main")` just works (built as an object target: nothing is lost to static-library dead-stripping, no plugin/backing-target split). `project.QtTranslations(...)` compiles `.ts` catalogs with lrelease and embeds the `.qm` files; `ninja lupdate` refreshes catalogs from sources as an explicit utility target. `project.QtDeploy(...)` wires `ninja deploy` to macdeployqt (on a `.app` bundle) or windeployqt. Utility targets use the new `Target.build_by_default = False`, which excludes any target from `ninja all` and implicit defaults — useful beyond Qt for doc generation, formatters, and other run-on-request tools.
- **First-class Qt 6 support** (`docs/qt.md`): a Qt Widgets app is now a five-line build script. `find_qt(project, env, modules=["Widgets"])` locates Qt via pkg-config or qtpaths introspection (Linux distro packages, Homebrew, the official installer, Windows) and returns modules as linkable targets with all platform quirks baked in — macOS framework linking, MSVC's required `/Zc:__cplusplus /permissive-`, Windows debug `d`-suffix libraries, and the Apple-Silicon `qyieldcpu.h` workaround for Qt < 6.10. `project.QtProgram(...)` (and `QtSharedLibrary`/`QtStaticLibrary`) accepts `.ui` and `.qrc` files directly in `sources` and finds `Q_OBJECT` classes itself. Unlike CMake's AUTOMOC there is no build-time scanning, no opaque `_autogen` step, and no `mocs_compilation.cpp` aggregate: every moc/uic/rcc run is a visible ninja edge, incrementally correct via the tools' own depfiles, and each moc output compiles as its own translation unit. The generate-time scan is guarded by a cheap build edge that fails with a clear "re-run pcons" message if a header gains `Q_OBJECT` later; a `.cpp` with `Q_OBJECT` but no `#include "foo.moc"` is a generate-time error with the exact line to add. `project.QtResources(...)` embeds files from a Python list — no `.qrc` XML. Low-level builders (`env.qt.Moc/Uic/Rcc`) expose each step individually. New examples `52_qt_widgets` and `53_qt_explicit`.
