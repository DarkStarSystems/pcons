...
- **`env.cxx.set_standard("c++20")` / `env.cxx.set_stdlib("libc++")`** select the C++ standard and standard library portably, as tool-facet knobs (no C/C++ vocabulary in core). Each toolchain maps them to the right flags — MSVC (which has no `/std:c++23`) maps C++23/26 to `/std:c++latest`, and `set_stdlib` is clang-only (no-op on GCC/MSVC). Together they replace the hand-written `if toolchain.name == "msvc"` standard/stdlib branches across the C++ examples.
