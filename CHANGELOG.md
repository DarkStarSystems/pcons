...
- **Swift support** (`toolchain="swift"`): Swift programs and libraries with whole-module compilation — each target compiles as one Swift module in a single `swiftc` invocation, so files in a target see each other without imports, and importing another target's module is an ordinary dependency (its `.swiftmodule` search path propagates as a usage requirement). Debug/release variants, `werror`, per-file `compile_commands.json` entries for sourcekit-lsp, and `pcons init` adoption of `.swift` sources are all wired in. New examples `46_swift_hello` and `47_swift_library` (with a `Test()` target). Requires Xcode on macOS or a swift.org toolchain on Linux; Windows Swift is not yet supported. Under the hood this adds a tool-agnostic whole-module mechanism (`SourceHandler.group_sources`) reusable by any module-at-once language.
