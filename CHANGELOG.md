...
- **`env.set_cxx_standard("c++20")`** selects the C++ standard portably — each toolchain maps it to the right flag (`-std=c++20` / `/std:c++20`), and MSVC (which has no `/std:c++23`) maps C++23/26 to `/std:c++latest`. Replaces the hand-written `if toolchain.name == "msvc"` branches across the C++ examples.
