...
**pcons** is C/C++-focused in practice, with GCC, Clang/LLVM, MSVC, and Clang-CL supported, including C++20 modules. Fortran is also built-in. Specialized toolchains for CUDA, Cython, WebAssembly (WASI + Emscripten), Metal shaders, and Windows RC files. A contrib LaTeX toolchain (via `latexmk`) demonstrates that the architecture handles non-compiled workflows with multi-pass builds, bibliography, and index generation. The architecture is fully extensible via a toolchain registry, so adding new languages is a first-class operation. Users can also easily add tools, toolchains and builders. Polyglot projects *should* work well and are a design goal, but it is still early days.
